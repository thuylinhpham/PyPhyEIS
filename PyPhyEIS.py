...
class RunFitting(QThread):
    ...
    def fit_leastsquares(self):
        ...
        self.p_cov = p_cov * models.cost_scalar(self.params_ret, self.zp_data, self.fp_data,
                                                self.recv_wgt_index, self.calc_func, self.tp_data,
                                                self.volp_data, self.guess, self.guess_names,
                                                self.params_dict) / (2*self.zp_data.size - self.init_val.size)
    ...
    def run(self):
        ...
        if self.isFit == 1:
            ...
            if self.p_cov is not None:
                ...
                self.perror = ['N/A'] * len(self.params_ret)
                self.perror_percent = ['N/A'] * len(self.params_ret)
            ...
            self.reduced_chisqr = self.chisqr / (2*self.zp_data.size - self.init_val.size)
            resd = self.chisqr * (2*self.zp_data.size - self.init_val.size)
...
class FittingImpedance(QObject):
    def __init__(self, rootObj=None):
        ...
        self.default_niters = 100000                       
        self.niters = self.default_niters
    ...
    def done(self):
        ...
        if self.runFittingSimulation == 2:
            ...
            self.chiSquare.emit('NaN', 'NaN', 'NaN')
    ...
    def loadParameters(self, load_path):
        ...
        ld_names = df_pars[0, :]
        if "Data name" in ld_names:
            st_index = 3
        elif "chi-square" in ld_names:
            st_index = 2
        else:
            st_index = 0
            
        df_names = df_pars[0, st_index:]
        df_values = df_pars[1, st_index:]
    ...
    def saveParameters(self, saved_path):
        ...
        cols = ["Data name", "chi-square", "sum of square"] + cols
        cols_values = [self.recv_data, self.result['chisqr'], self.result['residual']] + cols_values                                                            
    ...
    def parse_params_ret(self):
        ...
        for idx in range(len(self.recv_names)):
            ...
            if name in self.guess_names:
                ...
                if perror[cur_idx] != '' and perror[cur_idx] != 'N/A':                
                    ret_error[idx] = '{:12.6e}'.format(float(perror[cur_idx]))
                    ret_error_percent[idx] = '{:12.6e}'.format(float(perror_percent[cur_idx]))
    ...
    def dx19_read_data(self, dataPath):
        """
        Load dx30 data from dataPath
        :param dataPath:
        :return:
        """
        df = pd.read_csv(dataPath, sep="\t", header=None, skiprows=7).values
        # print(df)
        f_data = df[:, 0]
        z_data = df[:, 1] + 1j * df[:, 2]
...
if __name__ == "__main__":
    ...
    app.setApplicationName("PyPhyEIS by Thuy Linh Pham")
